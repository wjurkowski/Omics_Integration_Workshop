#########################################################################################
##########         Multivariate methods for Multiomics Analysis          ##########
#########################################################################################
...
setwd("~/Trabajo/Docencia/Norwich/")

# PCA example  -------------------------------------

load("data.M.RData")
colnames(data.M)
head(rownames(data.M))
dim(data.M)
PCA <- princomp(data.M, cor = TRUE) ## PCA
names(PCA)
PCA$loadings[,1]
head(PCA$scores)
PCA$sdev
...
# Creating a function to be re-used
myPCA.plot <- function (PCA, main, col = rep(c(1:8),each = 3)) {
  P <- PCA$loadings
  TOTvar <- cumsum(PCA$sdev^2/sum(PCA$sdev^2))
...
myPCA.plot(PCA, main = "stress response in SIPK-silenced plants")

## negative values on X axis indicate that the data is not centered
data.M1 <- t(apply(data.M, 1, scale , scale = FALSE))
colnames(data.M1) <- colnames(data.M)
PCA1 <- princomp(data.M1)
myPCA.plot(PCA1, main = "stress response in SIPK-silenced plants")

### Extract most representative genes ###
#########################################
T <- PCA1$scores
P <- PCA1$loadings
mega.genes <- T[,1] > 2*PCA1$sdev[1] & T[,2] < -2*PCA1$sdev[2]
control.genes <- T[,1] < -2*PCA1$sdev[1] & T[,2] < -2*PCA1$sdev[2]
mega.genes.UP <- names(which(mega.genes))
control.genes.DOWN <- names(which(control.genes))

## Plotting genes ##
#####################

# We need first to rescale loadings to scores scale.
rescale <- function(vector, oldmin = min(vector), oldmax = max(vector), newmin = 0, newmax = 100) 
{ 
  rango <- oldmax - oldmin 
  dimen <- dim(vector) 
...
T[,1] <- rescale(T[,1], newmin = min(P[,1]), newmax = max(P[,1]))
T[,2] <- rescale(T[,2], newmin = min(P[,2]), newmax = max(P[,2]))

points(T[mega.genes,1:2], col = "red", pch = 19, cex = 0.7)
points(T[control.genes,1:2], col = "dark green", pch = 19,  cex = 0.7)
...
# in keep you indicate the number of variables to keep for each Mode (X or Y) for each number of components.
...
# Run the code below, I already did
...
# this below takes time, I have already run it!
